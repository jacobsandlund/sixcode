...
i8 ui_all_initialize(UiAll *ui, i32 styles_buffer_capacity_max)
{
	...
	initialized[0] = ui_grid_initialize(&ui->grid, styles_buffer_capacity_max);
	initialized[1] = ui_fill_initialize(&ui->fill, &ui->grid);
	initialized[2] = ui_stroke_initialize(&ui->stroke, &ui->grid);
